...
import { ParticleLauncher } from './js/base/ParticleEffect'
...
class Demo {
    ...
    create() {
        ...
        //创建一个粒子发射器
        this.particleLauncher = new ParticleLauncher();
        this.particleLauncherConfig = {//使用粒子发射器创建粒子的时候  创建粒子的参数
            imageList: ['B2', 'B3', 'B4', 'B5', 'B1', 'B1', 'B1'],//图片key值  对应的粒子发生器中的值 当时多数是便使用随机
            position: { x: 300, y: 300 },//粒子出现坐标
            count: [5, 5],//数量 上线下线
            scale: [0.5, 1],//设置随机大小倍数
            gravity: 0.2,//重力的设置 
            rotate: [-3, 3],//随机旋转的角度
            speed: [1, 5],//速度
            direction: [1, 1],//向量  上下  如果你想调试相关的参数 可以在重力 与向量中一个设为0 这样容易出效果
        };
        this.particleLauncher.create(this.particleLauncherConfig);//玻璃渣生成特效
